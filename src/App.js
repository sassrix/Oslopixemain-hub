...
export default function OsloPixelHub() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: BG, fontFamily: "'DM Sans','Helvetica Neue',sans-serif", color: "#f0ebe0", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "fixed", bottom: -100, right: -60, width: 350, height: 350, borderRadius: "50%", background: `radial-gradient(circle, ${CORAL}06 0%, transparent 70%)`, pointerEvents: "none", zIndex: 0 }} />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", zIndex: 5 }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "0 16px" : "0 28px", height: isMobile ? 52 : 56, borderBottom: `1px solid ${BORDER}`, background: "rgba(13,13,15,0.85)", backdropFilter: "blur(12px)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: dbReady ? GREEN : "#444", transition: "background 0.5s" }} title={dbReady ? "Connected" : "Connecting..."} />
...
function Dashboard({ isMobile, mounted, setActive, dbReady }) {
  const [stats, setStats] = useState({ leads: 0, projects: 0, content: 0, ideas: 0 });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      db.from("content").select("status"),
      db.from("ideas").select("status"),
      db.from("activity").select("*").order("created_at", { ascending: false }).limit(7),
    ]).then(([leads, projects, content, ideas, act]) => {
      setStats({
        leads: leads.data?.length || 0,
        projects: projects.data?.filter(p => p.status === "active" || p.status === "in-progress").length || 0,
        content: content.data?.filter(c => c.status !== "published").length || 0,
        ideas: ideas.data?.filter(i => i.status === "exploring" || i.status === "testing").length || 0,
      });
  ...
  const statCards = [
    ...
    { label: "Content in Queue", value: stats.content, color: GOLD, id: "marketing" },
    { label: "Ideas in Progress", value: stats.ideas, color: CORAL, id: "rd" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
        {statCards.map((s, i) => (
          <div key={s.label} onClick={() => setActive(s.id)} style={{ ...anim(i * 0.07), background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "16px", cursor: "pointer" }}>
            ...
            <div style={{ fontSize: isMobile ? 24 : 28, fontWeight: 600, letterSpacing: -0.5 }}>{s.value}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 300px", gap: 24 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            {activity.length === 0
              ? <div style={{ fontSize: 12, color: "#333" }}>No activity yet.</div>
              : activity.map(a => (
...
function IdeasTracker({ isMobile }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await db.from("ideas").select("*").order("potential").order("created_at", { ascending: false });
    setItems(data || []);
    setLoading(false);
  }, []);
  ...
  const save = async () => {
    ...
    if (selected) {
      await db.from("ideas").update({ ...form, updated_at: new Date().toISOString() }).eq("id", selected.id);
    } else {
      await db.from("ideas").insert(form);
      await db.from("activity").insert({ text: `New idea: ${form.title}`, module: "rd", type: "new" });
    }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <p style={{ fontSize: 12.5, color: "#555", margin: 0 }}>{items.length} ideas · {counts.adopted || 0} adopted</p>
        <Btn onClick={() => { setSelected(null); setForm(blank); setShowForm(true); }} color={CORAL} small>+ Add Idea</Btn>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {["all", ...IDEA_STATUSES].map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid ${filter === s ? (STATUS_COLORS[s] || CORAL) : BORDER}`, background: filter === s ? `${STATUS_COLORS[s] || CORAL}15` : "transparent", color: filter === s ? (STATUS_COLORS[s] || CORAL) : "#555", fontSize: 11, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#444" }}>Loading...</div>
        : filtered.length === 0 ? <EmptyState icon="◌" text="No ideas yet" sub='Click "+ Add Idea" to capture your first one' />
        : (
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, 1fr)", gap: 10 }}>
            {filtered.map(item => (
              <div key={item.id} onClick={() => openEdit(item)} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "16px", cursor: "pointer", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: "#f0ebe0", flex: 1, paddingRight: 8, lineHeight: 1.4 }}>{item.title}</div>
                  <button onClick={e => { e.stopPropagation(); remove(item.id, item.title); }} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", fontSize: 16, padding: 0, flexShrink: 0 }}>×</button>
                </div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: item.description ? 10 : 0 }}>
                  <Pill label={item.status} color={STATUS_COLORS[item.status] || "#555"} />
                  <Pill label={item.category} color={IDEA_CATEGORY_COLORS[item.category] || "#555"} />
                  <Pill label={`${item.potential} potential`} color={POTENTIAL_COLORS[item.potential]} />
                </div>
      ...
      {showForm && (
        <Modal title={selected ? "Edit Idea" : "New Idea"} onClose={() => { setShowForm(false); setSelected(null); }} color={CORAL}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Input label="Title *" value={form.title} onChange={v => setForm(f => ({ ...f, title: v }))} placeholder="e.g. Monthly GBP retainer" />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              <Input label="Category" value={form.category} onChange={v => setForm(f => ({ ...f, category: v }))} options={IDEA_CATEGORIES} />
              <Input label="Status" value={form.status} onChange={v => setForm(f => ({ ...f, status: v }))} options={IDEA_STATUSES} />
              <Input label="Potential" value={form.potential} onChange={v => setForm(f => ({ ...f, potential: v }))} options={IDEA_POTENTIALS} />
            </div>
            <Input label="Description" value={form.description} onChange={v => setForm(f => ({ ...f, description: v }))} type="textarea" placeholder="What is this idea? Why does it matter?" />
            <Input label="Notes" value={form.notes} onChange={v => setForm(f => ({ ...f, notes: v }))} type="textarea" placeholder="Research, risks, next steps..." />
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
              <Btn onClick={() => { setShowForm(false); setSelected(null); }} variant="neutral">Cancel</Btn>
              <Btn onClick={save} color={CORAL}>{selected ? "Save Changes" : "Add Idea"}</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function CompetitorTracker({ isMobile }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await db.from("competitors").select("*").order("created_at", { ascending: false });
    setItems(data || []);
    setLoading(false);
  }, []);
  ...
  const save = async () => {
    ...
    if (selected) {
      await db.from("competitors").update({ ...form, updated_at: new Date().toISOString() }).eq("id", selected.id);
    } else {
      await db.from("competitors").insert(form);
      await db.from("activity").insert({ text: `Competitor added: ${form.name}`, module: "rd", type: "new" });
    }
    setShowForm(false); setSelected(null); setForm(blank); load();
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <p style={{ fontSize: 12.5, color: "#555", margin: 0 }}>{items.length} competitors tracked</p>
        <Btn onClick={() => { setSelected(null); setForm(blank); setShowForm(true); }} color={CORAL} small>+ Add Competitor</Btn>
      </div>

      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#444" }}>Loading...</div>
        : items.length === 0 ? <EmptyState icon="◌" text="No competitors yet" sub="Track who you're up against in Oslo" />
        : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {items.map(item => (
              <div key={item.id} onClick={() => openEdit(item)} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "16px", cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 500, color: "#f0ebe0", marginBottom: 3 }}>{item.name}</div>
                    <div style={{ display: "flex", gap: 10 }}>
                      {item.location && <span style={{ fontSize: 11, color: "#444" }}>📍 {item.location}</span>}
                      {item.website && <a href={item.website.startsWith("http") ? item.website : `https://${item.website}`} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: 11, color: CORAL, textDecoration: "none" }}>Website →</a>}
                    </div>
                  </div>
                  <button onClick={e => { e.stopPropagation(); remove(item.id, item.name); }} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", fontSize: 16, padding: 0 }}>×</button>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
                  {item.strengths && (
                    <div style={{ background: `${GREEN}08`, border: `1px solid ${GREEN}18`, borderRadius: 8, padding: "10px 12px" }}>
                      <div style={{ fontSize: 9.5, color: GREEN, letterSpacing: 1, textTransform: "uppercase", marginBottom: 5 }}>Strengths</div>
                      <div style={{ fontSize: 12, color: "#aaa", lineHeight: 1.5 }}>{item.strengths}</div>
                    </div>
                  )}
                  {item.weaknesses && (
                    <div style={{ background: `${CORAL}08`, border: `1px solid ${CORAL}18`, borderRadius: 8, padding: "10px 12px" }}>
                      <div style={{ fontSize: 9.5, color: CORAL, letterSpacing: 1, textTransform: "uppercase", marginBottom: 5 }}>Weaknesses</div>
                      <div style={{ fontSize: 12, color: "#aaa", lineHeight: 1.5 }}>{item.weaknesses}</div>
                    </div>
                  )}
                </div>
      ...
      {showForm && (
        <Modal title={selected ? "Edit Competitor" : "New Competitor"} onClose={() => { setShowForm(false); setSelected(null); }} color={CORAL}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Input label="Name *" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder="Agency Oslo AS" />
              <Input label="Location" value={form.location} onChange={v => setForm(f => ({ ...f, location: v }))} placeholder="Oslo, Norway" />
            </div>
            <Input label="Website" value={form.website} onChange={v => setForm(f => ({ ...f, website: v }))} placeholder="agencyoslo.no" />
            <Input label="Pricing" value={form.pricing} onChange={v => setForm(f => ({ ...f, pricing: v }))} placeholder="e.g. NOK 15,000–40,000 for websites" />
            <Input label="Strengths" value={form.strengths} onChange={v => setForm(f => ({ ...f, strengths: v }))} type="textarea" placeholder="What are they good at?" />
            <Input label="Weaknesses" value={form.weaknesses} onChange={v => setForm(f => ({ ...f, weaknesses: v }))} type="textarea" placeholder="Where do they fall short?" />
            <Input label="Notes" value={form.notes} onChange={v => setForm(f => ({ ...f, notes: v }))} type="textarea" placeholder="Observations, opportunities..." />
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
              <Btn onClick={() => { setShowForm(false); setSelected(null); }} variant="neutral">Cancel</Btn>
              <Btn onClick={save} color={CORAL}>{selected ? "Save Changes" : "Add Competitor"}</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function MarketingModule({ isMobile }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 500, margin: "0 0 16px", color: GOLD }}>Marketing</h1>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <SectionTab label="Content Calendar" active={tab === "content"} onClick={() => setTab("content")} color={GOLD} />
          <SectionTab label="SEO Tracker" active={tab === "seo"} onClick={() => setTab("seo")} color={GOLD} />
          <SectionTab label="Brand Assets" active={tab === "assets"} onClick={() => setTab("assets")} color={GOLD} />
        </div>
      </div>
...
function ContentCalendar({ isMobile }) {
  ...
  const save = async () => {
    if (!form.title.trim()) return;
    const payload = { ...form, scheduled_date: form.scheduled_date || null, published_date: form.published_date || null };
    if (selected) { await db.from("content").update({ ...payload, updated_at: new Date().toISOString() }).eq("id", selected.id); }
    else { await db.from("content").insert(payload); await db.from("activity").insert({ text: `New content: ${form.title} (${form.platform})`, module: "marketing", type: "new" }); }
    setShowForm(false); setSelected(null); setForm(blank); load();
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {["all", ...CONTENT_STATUSES].map(s => (<button key={s} onClick={() => setFilter(s)} style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid ${filter === s ? (STATUS_COLORS[s] || GOLD) : BORDER}`, background: filter === s ? `${STATUS_COLORS[s] || GOLD}15` : "transparent", color: filter === s ? (STATUS_COLORS[s] || GOLD) : "#555", fontSize: 11, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>{s === "all" ? `All (${items.length})` : `${s} (${counts[s] || 0})`}</button>))}
      </div>
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#444" }}>Loading...</div> : filtered.length === 0 ? <EmptyState icon="◈" text="No content yet" sub='Click "+ Add" to plan your first piece' /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map(item => (<div key={item.id} onClick={() => openEdit(item)} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
            <div style={{ fontSize: 16, color: PLATFORM_COLORS[item.platform] || "#666", width: 34, height: 34, border: `1px solid ${PLATFORM_COLORS[item.platform] || "#666"}28`, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{PLATFORM_ICONS[item.platform] || "◌"}</div>
            <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14, fontWeight: 500, color: "#f0ebe0", marginBottom: 4, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.title}</div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Pill label={item.status} color={STATUS_COLORS[item.status]} /><span style={{ fontSize: 11, color: "#444", textTransform: "capitalize" }}>{item.platform} · {item.content_type}</span>{item.scheduled_date && <span style={{ fontSize: 11, color: GOLD }}>Sched. {new Date(item.scheduled_date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>}</div></div>
            <button onClick={e => { e.stopPropagation(); remove(item.id, item.title); }} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", fontSize: 16, padding: 4, flexShrink: 0 }}>×</button>
          </div>))}
        </div>)}
...
function SEOTracker({ isMobile }) {
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#444" }}>Loading...</div> : items.length === 0 ? <EmptyState icon="◌" text="No keywords yet" sub="Add keywords you want to rank for" /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {items.map(item => (<div key={item.id} onClick={() => openEdit(item)} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
            <div style={{ flex: 1, minWidth: 0 }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}><span style={{ fontSize: 14, fontWeight: 500, color: "#f0ebe0" }}>{item.keyword}</span><Pill label={item.priority} color={PRIORITY_COLORS[item.priority]} /></div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>{item.location && <span style={{ fontSize: 11, color: "#444" }}>📍 {item.location}</span>}{item.current_rank && <span style={{ fontSize: 11, color: "#555" }}>Rank: <span style={{ color: "#ccc" }}>{item.current_rank}</span></span>}{item.target_rank && <span style={{ fontSize: 11, color: "#555" }}>Target: <span style={{ color: GREEN }}>{item.target_rank}</span></span>}{item.search_volume && <span style={{ fontSize: 11, color: "#555" }}>Vol: <span style={{ color: BLUE }}>{item.search_volume}</span></span>}</div>{item.notes && <div style={{ fontSize: 11.5, color: "#444", marginTop: 5 }}>{item.notes}</div>}</div>
            <button onClick={e => { e.stopPropagation(); remove(item.id, item.keyword); }} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", fontSize: 16, padding: 4, flexShrink: 0 }}>×</button>
          </div>))}
        </div>)}
...
function BrandAssets({ isMobile }) {
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#444" }}>Loading...</div> : items.length === 0 ? <EmptyState icon="◈" text="No assets yet" sub="Add links to your logos, photos, and templates" /> : Object.entries(grouped).map(([cat, catItems]) => (
        <div key={cat} style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 10, letterSpacing: 1.8, textTransform: "uppercase", color: "#333", marginBottom: 10 }}>{cat}</div>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, 1fr)", gap: 8 }}>
            {catItems.map(item => (<div key={item.id} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ fontSize: 16, color: ASSET_COLORS[item.category] || "#666", width: 34, height: 34, border: `1px solid ${ASSET_COLORS[item.category] || "#666"}28`, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{ASSET_ICONS[item.category] || "◌"}</div>
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 500, color: "#f0ebe0", marginBottom: 2 }}>{item.name}</div>{item.description && <div style={{ fontSize: 11.5, color: "#555", marginBottom: 4 }}>{item.description}</div>}{item.url && <a href={item.url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: 11, color: GOLD, textDecoration: "none" }}>Open link →</a>}</div>
              <div style={{ display: "flex", gap: 4, flexShrink: 0 }}><button onClick={() => openEdit(item)} style={{ background: "none", border: "none", color: "#444", cursor: "pointer", fontSize: 13, padding: 4 }}>✎</button><button onClick={() => remove(item.id, item.name)} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", fontSize: 16, padding: 4 }}>×</button></div>
            </div>))}
          </div>
        </div>))}
          ...
          {filtered.map(project => { const { done, total } = getProgress(project); return (
            <div key={project.id} onClick={() => setOpenProject(project)} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "18px", cursor: "pointer", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
                <div><div style={{ fontSize: 15, fontWeight: 500, color: "#f0ebe0", marginBottom: 4 }}>{project.client_name}</div>{project.business && <div style={{ fontSize: 12, color: "#555" }}>{project.business}</div>}</div>
                <button onClick={e => { e.stopPropagation(); openEdit(project); }} style={{ background: "none", border: "none", color: "#444", cursor: "pointer", fontSize: 13, padding: 4 }}>✎</button>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                {project.due_date ? <span style={{ fontSize: 11, color: "#444" }}>Due {new Date(project.due_date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span> : <span />}
                <div style={{ display: "flex", gap: 8 }}><span style={{ fontSize: 11, color: PURPLE }}>View →</span><button onClick={e => { e.stopPropagation(); remove(project.id, project.client_name); }} style={{ background: "none", border: "none", color: "#333", cursor: "pointer", fontSize: 14, padding: 0 }}>×</button></div>
              </div>
            </div>);})}
        </div>)}
